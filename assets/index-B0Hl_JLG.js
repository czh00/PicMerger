(async ()=>{
    ...
    const V = "modulepreload", Z = function(n, t) {
        ...
        if (a && a.length > 0) {
            ...
            const g = document.getElementsByTagName("link"), b = document.querySelector("meta[property=csp-nonce]"), v = b?.nonce || b?.getAttribute("nonce");
            r = f(a.map((m)=>{
                if (m = Z(m, i), m in D) return;
                D[m] = !0;
                const c = m.endsWith(".css"), s = c ? '[rel="stylesheet"]' : "";
                if (!!i) for(let u = g.length - 1; u >= 0; u--){
                    const p = g[u];
                    if (p.href === m && (!c || p.rel === "stylesheet")) return;
                }
                else if (document.querySelector(`link[href="${m}"]${s}`)) return;
                const h = document.createElement("link");
                if (h.rel = c ? "stylesheet" : V, c || (h.as = "script"), h.crossOrigin = "", h.href = m, v && h.setAttribute("nonce", v), document.head.appendChild(h), c) return new Promise((u, p)=>{
                    h.addEventListener("load", u), h.addEventListener("error", ()=>p(new Error(`Unable to preload CSS for ${m}`)));
                });
            }));
        }
        function l(f) {
        ...
        return r.then((f)=>{
            for (const g of f || [])g.status === "rejected" && l(g.reason);
            return t().catch(l);
        });
    };
    ...
    function J() {
        ...
        n && n.querySelectorAll("button").forEach((t)=>{
            t.addEventListener("click", ()=>{
                o.outputMode = t.dataset.value, o.outputScale = 100, n.querySelectorAll("button").forEach((a)=>a.classList.remove("active")), t.classList.add("active"), C(), E();
            });
        ...
        for (const a of Array.from(n))if (a.type.startsWith("image/") || /\.(jpg|jpeg|png|webp|gif|bmp)$/i.test(a.name)) try {
            const r = await new Promise((l, f)=>{
                ...
                g.onload = ()=>{
                    ...
                    v.width = g.width, v.height = g.height, v.getContext("2d").drawImage(g, 0, 0, v.width, v.height), v.toBlob((c)=>{
    ...
    function I() {
        ...
        `, a.querySelector(".btn-remove").addEventListener("click", (i)=>{
            ...
            }), a.addEventListener("dragstart", $), a.addEventListener("dragover", W), a.addEventListener("drop", N), a.addEventListener("dragend", O), a.addEventListener("dragenter", H), a.addEventListener("dragleave", A), a.addEventListener("touchstart", j, {
                passive: !1
    ...
    function N(n) {
        ...
        if (y !== t) {
            ...
            o.images.splice(t, 0, a), I(), R(), e.downloadSection.style.display === "block" && E();
        }
        return !1;
    }
    function O(n) {
    ...
    function _(n) {
        ...
        L && L !== i && L.classList.remove("drag-over"), i && i.dataset.index !== void 0 && parseInt(i.dataset.index) !== y ? (i.classList.add("drag-over"), L = i) : L = null;
    }
    function z(n) {
        if (this.classList.remove("dragging"), this.style.pointerEvents = "auto", L) {
            const t = parseInt(L.dataset.index);
            if (L.classList.remove("drag-over"), y !== null && !isNaN(t) && y !== t) {
                const a = o.images.splice(y, 1)[0];
                o.images.splice(t, 0, a), I(), R(), e.downloadSection.style.display === "block" && E();
            }
        }
        L = null, y = null, document.querySelectorAll(".image-item, .sort-item").forEach((t)=>{
            t.classList.remove("dragging", "drag-over"), t.style.pointerEvents = "auto";
        });
    }
    function B(n, t, a, i, r, l) {
        const f = t.width / t.height, g = r / l;
        let b, v, m, c;
        f > g ? (b = r, v = r / f, m = a, c = i + (l - v) / 2) : (v = l, b = l * f, m = a + (r - b) / 2, c = i), n.drawImage(t, 0, 0, t.width, t.height, m, c, b, v);
    }
    function E() {
        ...
        if (l === "horizontal") {
            ...
            g = o.scaleMode === "fit-first" ? n[0].width : Math.max(...n.map((s)=>s.width)), t = g * i;
            ...
            for(let s = 0; s < c; s++){
                ...
                for(let h = 0; h < i; h++){
                    const u = s * i + h;
                    if (u < r) {
                        const p = n[u], w = p.height * (g / p.width);
                        w > d && (d = w);
                    }
                }
                f.push(d);
        ...
        if (e.canvas.width = v, e.canvas.height = m, e.canvas.style.width = v + "px", e.ctx.fillStyle = o.bgColor, e.ctx.fillRect(0, 0, v, m), e.ctx.save(), e.ctx.scale(b, b), l === "grid") {
            ...
            n.forEach((s)=>{
                let d, h, u, p;
                l === "horizontal" ? (h = a, d = s.width * (a / s.height), u = c, p = 0, c += d) : (d = t, h = s.height * (t / s.width), u = 0, p = c, c += h), B(e.ctx, s.img, u, p, d, h);
            });
        }
        e.ctx.restore(), e.downloadSection.style.display = "block", e.previewInfo.textContent = `即時預覽中... 解析度: ${Math.round(v)} x ${Math.round(m)}`, C();
    }
    async function Q() {
        console.log("PicMerger v1.0.15 Initializing..."), Y(), U(), e.btnShare && (window.Capacitor && window.Capacitor.isNativePlatform() && o.canShare ? (e.btnShare.style.setProperty("display", "block", "important"), e.btnShare.disabled = !1) : e.btnShare.style.setProperty("display", "none", "important")), J(), console.log("Event listeners attached.");
        try {
        ...
        if (!(o.images.length < 1)) {
            ...
            try {
                ...
                if (o.direction === "grid" && (o.gridCols === 1 ? t = n.vertical : o.gridCols === o.images.length ? t = n.horizontal : a = !0), a || !x) {
                    console.info("使用 JS 高品質引擎渲染 (Contain)..."), E();
                ...
                const g = parseInt(o.bgColor.slice(1, 3), 16), b = parseInt(o.bgColor.slice(3, 5), 16), v = parseInt(o.bgColor.slice(5, 7), 16), m = performance.now(), c = x(r, l, t, 1, o.gridCols, g, b, v), s = performance.now(), d = new Blob([
                    c
                ], {
                    type: "image/png"
                }), h = URL.createObjectURL(d), u = new Image;
                u.onload = ()=>{
                    o.baseWidth = u.width, o.baseHeight = u.height, e.canvas.width = u.width, e.canvas.height = u.height, e.ctx.drawImage(u, 0, 0), e.downloadSection.style.display = "block", e.btnShare && (e.btnShare.disabled = !o.canShare), e.previewInfo.textContent = `高品質 Rust 引擎處理完成！解析度: ${u.width}x${u.height} (耗時 ${(s - m).toFixed(0)}ms)`, e.btnMerge.disabled = !1, C();
                }, u.src = h;
            } catch (n) {
