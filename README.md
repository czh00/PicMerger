# PicMerger (圖片合併) v1.1.0

一個極簡、高品質的圖片與影片合併工具，支援 Android 與 Web 平台。

## 🌟 v1.1.0 重大更新
- **無縫動態網格 (Seamless Dynamic Grid)**：自動根據圖片比例計算行高，消除黑邊與縫隙，達成完美填充效果。
- **實時尺寸預算 (Real-time Dimensions)**：調整參數時即時顯示最終輸出的預估解析度，精準掌握品質。
- **穩定拖拽排序 (Stable Drag-and-Drop)**：Android 版重構座標系統，實現「指尖鎖定」精確操控，不再偏移。
- **全域重置功能 (Global Reset)**：一鍵恢復所有佈局、列數、比例、背景色等選項至預設值。
- **操控手感優化**：優化預覽區域的縮放與平移靈敏度，操控更平穩細膩。

## 🚀 歷史更新 (v1.0.4)
- **修正影片合併**：修復了特定格式下影片合併失敗的問題。
## 主要功能
...
- `PicMerger_v1.1.0.apk`: 最新發佈的 Android 安裝包。
